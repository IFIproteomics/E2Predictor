...
read_file_add_filename <- function(thefile, keepFolderName = F, UseGenericHeaders=T){
    
    if(length(thefile)==0){
        return(NULL)
    }
    
    if(grepl("csv$", thefile)){
        csvdata <- read_csv(thefile)    
    }else{
        csvdata <- read_tsv(thefile)    
    }
    
    
    foldername <- strsplit(thefiles[1], "/")[[1]]
